...
//        Inputs for the SoftSampler
   ...
   size_t T      = 10000000;     // number of MCMC steps
...
// --------------------------------------------KEY PARAMETERS FOR SAMPLER---------------------------------------------------------
   ...
   double kc  = 1.0;               // factor for proportionality constant in density w.r.t. surface measure
   
   double ks  = 0.4;                // factor for Soft proposal size
...
// -------------------------------------------------------------------------------------------------------------------------------
   ...
   SoftSampler(Schain, &stats, T, eps, p_hard, p_soft, x, M, cf,
...
// Check angle of sample with respect to simmetry axis
   ...
   double dt = PI/( (double) ntbins);   // bin size
...
// Write Schain in binary format (in a file called "Schain.bin"), much faster for Python to read when Schain is large
   size_t size1 = Ns*d;
